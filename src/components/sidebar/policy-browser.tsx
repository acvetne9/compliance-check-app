      ...
      <div className="flex flex-col gap-0.5 px-1">
        {filteredFolders.length === 0 ? (
          ...
          filteredFolders.map((folder) => {
            ...
            // Folder progress: sum of all doc progress / (count * 100) * 100
            // This tracks total requirements checked across all policies in folder
            const docsWithProgress = folderDocIds
              .map((id) => policyProgress.get(id))
              .filter((p): p is number => p !== undefined);
            const folderProgress = docsWithProgress.length > 0
              ? Math.round(docsWithProgress.reduce((a, b) => a + b, 0) / docsWithProgress.length)
              : null;
              ...
              >
                {folder.docs.map((doc) => (
                  <PolicyItem
                    ...
                    progress={policyProgress.get(doc.id) ?? null}
